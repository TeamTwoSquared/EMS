...
class helpReplyForClient extends Controller
{
    public function index(){
        $newHelpRequest = DB::table('notifications')->where('is_read',0)->where('type',1)->where('customer_id',session()->get('customer_id'))->get();
        $newHelpRequestComment = DB::table('notifications')->where('is_read',0)->where('type',2)->where('customer_id',session()->get('customer_id'))->get();
         //dd($newHelpRequest);
